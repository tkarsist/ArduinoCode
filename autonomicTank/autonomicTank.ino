...
// PESTUU TANK MUUTTUJAT
// tuntosarvi pins
int vasenSarvi=13;
int oikeaSarvi=8;

//motor pins
int r_motor_n = 6;  //PWM control Right Motor -
int r_motor_p = 11;  //PWM control Right Motor +
int l_motor_p = 10;   //PWM control Left Motor +
int l_motor_n = 9;   //PWM control Left Motor -
int incomingByte = 0;	// for incoming serial data

// lippu mihin sarveen osunut
int osumaSarveen=0;
// PESTUU TANK MUUTTUJAT LOPPUU
...
void loop() { 
  ...
  if(targetBearing!=0){
  if(!correctionOngoing&&(curTime-lastTime)>updateBearingInterval){
    lastTime=curTime;
...
void tankGoLeft() {
       ...
       analogWrite(r_motor_n, 190);  //Set motor direction, 1 low, 2 high
...
if(stringCommand=="BT"){
     bearingThreshold=getIntFromString(message);
}

if(stringCommand=="BI"){
     updateBearingInterval=getIntFromString(message);
}
