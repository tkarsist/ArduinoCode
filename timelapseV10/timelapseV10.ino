...
//todo taski: Lisaa lineaarinen kiihdytys ja hidastus
/*
Lisaa y const, joka on kuinka suuri osa on kiihdystysta: esim 1/5
 lisaa z const joka on kuinka suuri hidastus esim 1/5
 
 esim: 1000 steppia molemmat 1/5 --> 1-200 step kiihdytys, 201-800 tasaianen, 801-1000 hidastus
 startAcc=1/6
 stopAcc=1/6
 startAccSteps=startAcc*shots
 stopDecSteps=stopDec*shots
 Vavg=distance/shots
 Vmax=Vavg/(1/2*startAcc+(1-startAcc-stopDec)+1/2*stopDec)
 startAccStatic=Vmax/(startAccSteps)
 stopDecStatic=Vmax/(stopAccSteps)
 returnToHomeCounterLinear=++ aina ku tehdaan jotain
 
 pseudo: tee taski ja laske noi arvot valmiiksi
 
 //voi muuten laittaa suoraan mm to move sen sijaan etta tallentaisi structiin
 Moottoriajoluuppi
 if(repeatCount<startAccSteps)
 servo.mmToMove=repeatCount*startAccStatic
 
 if(repeatCount>startAccSteps && repeatCount<stopAccSteps)
 servo.mmToMove=Vmax
 
 if(repeatCount>(shots-stopAccSteps))
 servo1.mmToMove=(shots-repeatCount)*stopDecStatic
